<template>
    <div class="h-240">
        ...
        <el-scrollbar class="h-230" v-loading="loadchart">
            <svg id="Viz_vf" :height="height" :width="width"></svg>
        </el-scrollbar>
        <div class="w-310 h-60 bg-gray-200 flex justify-center">
            <svg id="Legend_vf" height="400" width="1140"></svg>
        </div>
    </div>
</template>
...
const vfdata = computed(() => {
    return phageStore.phagevf
})
...
const drawLegend = () => {
        // .style('top', `300px`)
        // .style('left', `800px`)
        const legnedSvg = d3.select('#Legend_vf')
        legnedSvg
...
const drawVFLegend = () => {
    const legendSvg = d3.select('#Viz_vf')
                        .append('svg')
                        .attr('x',width.value-200)
                        .attr('y',height.value-400)
    const vfType = ['Adherence', 'Antimicrobial activity/Competitive advantage','Biofilm', 'Effector delivery system', 'Exoenzyme', 'Exotoxin',
    ...
    legendSvg
        ...
        .on('mouseover', function (event, d) {
            // console.log(TypeDict[d])
            tooltip.text(vfTypeDict[d].name)
            return tooltip.style('visibility', 'visible')
        })
        .on('mousemove', function (event) {
            return tooltip.style('top', `${event.pageY}px`).style('left', `${event.pageX}px`)
...
const loadeddata = computed(() => {
    return phageStore.vfloaded && phageStore.phagedataloaded
})
...
watch(loadeddata, () => {
    ...
    const svgArea = d3.select('#Viz_vf')
    ...
    _.map(trna, (d) => {
        ...
        //get color
        const color = getVFColor(d.vf_category)
        // draw arrow
        ...
        const mouseenter = function () {
            ...
            const textContent=[`VF ID: ${d.vf_id}`,
                                `VF category: ${d.vf_category}`,
                                `Start Point: ${d.start}`,
                                `End Point: ${d.end}`,
                                `strand: ${d.strand}`,
                                ]
