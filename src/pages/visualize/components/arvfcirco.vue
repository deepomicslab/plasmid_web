<template>
    <div class="h-220">
        <div class="w-300 h-20 bg-gray-200 flex flex-row items-center justify-center">
        ...
        <el-scrollbar class="h-200" v-loading="loadchart">
            <svg id="Viz_arvf" :height="height" :width="width"></svg>
        </el-scrollbar>
    </div>
</template>
...
const arvfdata = computed(() => {
    return phageStore.phagearvf
})
...
const outerTypeDict = {
    vf: {
        name: 'Virulent Factor',
        color: '#7123f8',
    },
    ar: {
        name: 'Antimicrobial Resistance Gene',
        color: '#f94eba',
    },
}
...
const drawLegend = () => {
    const legendSvg = d3.select('#Viz_arvf')
                        .append('svg')
                        .attr('x',0)
...
const drawOuterLegend = () => {
    const legendSvg = d3.select('#Viz_arvf')
                        .append('svg')
                        .attr('x',width.value-285)
                        .attr('y',height.value-120)
...
const loadeddata = computed(() => {
    return phageStore.arvfloaded && phageStore.phagedataloaded
})
...
watch(loadeddata, () => {
    ...
    // if(typeof arvfdata.value==='undefined' 
    ...
    const arData = arvfdata.value['ar']
    const vfData = arvfdata.value['vf']

    const svgArea = d3.select('#Viz_arvf')
    ...
    _.map(protein, (d, i) => {
        ...
        const relatedAr = _.find(arData,(item)=>d.Protein_id === item.Protein_id)
        const relatedVf = _.find(vfData,(item)=>d.Protein_id === item.Protein_id)

        let color
        if(relatedAr){
            color = outerTypeDict['ar']['color']
        }
        else if(relatedVf){
            color = outerTypeDict['vf']['color']
        }else{
            color = shallowGray
        }
        ...
        if(relatedAr || relatedVf){
            thisArrow.attr('class','mark')
        }
        ...
        if(relatedVf){
            ...
            const mouseenter = function () {
                ...
                const radMid = (start_rad+end_rad)/2
                ...
                const textContent=[`ID: ${relatedVf.id}`,
                                    `Protein ID: ${relatedVf.Protein_id}`,
                                    `Source: ${relatedVf.Source}`, 
                                    `Aligned Protein in VFDB: ${relatedVf.Aligned_Protein_in_VFDB}`,
                                    `Dataset: ${relatedVf.dataset}`,                                    
                                ]
            ...
            const mouseenter = function () {
                ...
                const textContent=[`ID: ${relatedAr.id}`,
                                    `Protein ID: ${relatedAr.Protein_id}`,
                                    `Source: ${relatedAr.Source}`, 
                                    `Aligned Protein in CARD: ${relatedAr.Aligned_Protein_in_CARD}`,
                                    `Dataset: ${relatedAr.dataset}`,                                    
                                ]
                // console.log(textContent)
                toolArea.selectAll('text')
                        .data(toolTextPositions)
