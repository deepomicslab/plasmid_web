...
const loadeddata = computed(() => {
    return phageStore.argloaded && phageStore.phagedataloaded
})
...
watch(loadeddata, () => {
    ...
    const svgArea = d3.select('#Viz_arg')
    ...
    _.map(trna, (d) => {
        ...
        const mouseenter = function () {
            ...
            const textContent=[`ARG ID: ${d.id}`,
                                `Drug class: ${d.drug_class}`,
                                `Start Point: ${d.start}`,
                                `End Point: ${d.end}`,
                                `strand: ${d.strand}`,
                                ]
...
function downloadsvg() {
    const svgElement = document.getElementById('Viz_arg') as HTMLElement
