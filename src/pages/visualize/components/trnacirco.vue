<template>
    <div class="h-240">
        ...
        <el-scrollbar class="h-220" v-loading="loadchart">
            <svg id="Viz_trna" :height="height" :width="width"></svg>
        </el-scrollbar>
        <div class="w-310 h-85 bg-gray-200 flex justify-center">
            <svg id="Legend_trna" height="400" width="1140"></svg>
        </div>
    </div>
</template>
...
const trnadata = computed(() => {
    return phageStore.phagetrna
})
console.log(trnadata.value)
...
const getTRNAColor:string = (type: string)=>{
    return type.startsWith('tmRNA') ? 
    tRNATypeDict['tmRNA']['color'] : tRNATypeDict['tRNA']['color']
}
const drawLegend = () => {
    const legnedSvg = d3.select('#Legend_trna')
...
const drawTRNALegend = () => {
    const legendSvg = d3.select('#Viz_trna')
                        .append('svg')
                        .attr('x',width.value-120)
                        .attr('y',height.value-120)
    const tRNAType = ["tRNA","tmRNA"]
...
const loadeddata = computed(() => {
    return phageStore.trnaloaded && phageStore.phagedataloaded
})
...
watch(loadeddata, () => {
    ...
    const svgArea = d3.select('#Viz_trna')
    ...
    _.map(trna, (d) => {
        ...
        //get color
        const color = getTRNAColor(d.trna_type)
        // draw arrow
        ...
        const mouseenter = function () {
            ...
            const textContent=[`tRNA ID: ${d.trna_id}`,
                                `tRNA Type: ${d.trna_type}`,
                                `Start Point: ${d.start}`,
                                `Length: ${d.length}`,
                                `End Point: ${d.end}`,
                                `strand: ${d.strand}`,
...
function downloadsvg() {
    const svgElement = document.getElementById('Viz_trna') as HTMLElement
