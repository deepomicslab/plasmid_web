...
const transprotein = computed(() => {
    return phageStore.heliceData
})
...
const loaded = computed(() => {
    return phageStore.transproteinloaded && phageStore.phagedataloaded
})
...
// const drawLegend = () => {
//     const legendSvg = d3.select('#Viz_transprotein')
//                         .append('svg')
//                         .attr('x',0)
...
watch(loaded, () => {
    ...
    const svgArea = d3.select('#Viz_transprotein')
    ...
    _.map(transpro, (d, i) => {
        const pro = _.find(protein, 
            (item)=> d.protein_id === item.protein_id)
        let inside
        let tmHelix
        let outside

        if(!isNaN(parseInt(d.insidestart))){

            inside = svgArea.append('path')
            .attr('d', getSegmentPath(pro.start, 
                    d.insidestart, d.insideend))
            .attr('stroke', 'none')
            .attr('fill', outerTypeDict.inside.color)
            .attr('class','mark')
        }

        if(!isNaN(parseInt(d.TMhelixstart))){
            tmHelix = svgArea.append('path')
            .attr('d',getSegmentPath(pro.start, 
                    d.TMhelixstart, d.TMhelixend))
            .attr('stroke', 'none')
            .attr('fill',outerTypeDict.TMhelix.color)
            .attr('class','mark')
        }
        if(!isNaN(parseInt(d.outsidestart))){
            outside=svgArea.append('path')
            .attr('d',getSegmentPath(
                    pro.start,
                    d.outsidestart, d.outsideend
                ))
            .attr('stroke','none')
            .attr('fill',outerTypeDict.outside.color)
            .attr('class','mark')
        }

        //draw tooltip after drawing arrows
        ...
        if(inside){
            ...
            const mouseenter = function () {
                svgArea.selectAll('.mark').style('opacity', '0.3')
                d3.select(this).style('opacity', '1')
                
                const startEndRad = getStartEndRad(pro.start, 
                    d.insidestart, d.insideend)
                const radMid = (startEndRad[0]+startEndRad[1])/2
                ...
                const textContent=[
                                    `Inside start: ${d.insidestart}`,
                                    `Inside end: ${d.insideend}`,                                     
                                ]
            ...
            const mouseleave = function() {
                svgArea.selectAll('.mark').style('opacity', '1')
                toolArea
                    .style('opacity', 0).attr('x', 0).attr('y', 0)
                // toolArea.selectAll('text').style('opacity',0)
            }
            inside.on('mouseenter ', mouseenter).on('mouseleave', mouseleave)
        }
        ...
        if(tmHelix){
            ...
            const mouseenter = function () {
                ...
                const textContent=[
                                    `TMhelix start: ${d.TMhelixstart}`,
                                    `TMhelix end: ${d.TMhelixend}`,                                     
                                ]
                // console.log(textContent)
                toolArea.selectAll('text')
                        .data(toolTextPositions)
            ...
            tmHelix.on('mouseenter ', mouseenter).on('mouseleave', mouseleave)
        }
        ...
        if(outside){
            ...
            const mouseenter = function () {
                ...
                const startEndRad = getStartEndRad(
                    pro.start,
                    d.outsidestart, d.outsideend
                )
                const radMid = (startEndRad[0]+startEndRad[1])/2
                ...
                const textContent=[
                                    `Outside start: ${d.outsidestart}`,
                                    `Outside end: ${d.outsideend}`,                                     
                                ]
                // console.log(textContent)
                toolArea.selectAll('text')
                        .data(toolTextPositions)
            ...
            outside.on('mouseenter ', mouseenter).on('mouseleave', mouseleave)
        }
...
function downloadsvg() {
    const svgElement = document.getElementById('Viz_transprotein') as HTMLElement
    const svgData = new XMLSerializer().serializeToString(svgElement as SVGElement)
    const blob = new Blob([svgData], { type: 'image/svg+xml;charset=utf-8' })
    const url = URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = 'transpro_circo.svg'
    link.click()
...
const downloadSVGAsPNG = async () => {
    ...
    canvas.toBlob(blob => {
        if (blob) {
            ...
            const link = document.createElement('a')
            link.href = url
            link.download = 'transprotein_circo.png'
            link.click()
